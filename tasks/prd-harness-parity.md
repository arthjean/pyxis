...
- Le diff agrégé doit-il se limiter aux fichiers suivis par git ? Cela exclurait les fichiers ignorés, ce qui est souvent souhaitable pour un dépôt avec un répertoire de compilation volumineux, mais masquerait des modifications réelles. À trancher avant US-018, car cela détermine si une dépendance de surveillance du système de fichiers est nécessaire.
...
- Le schéma d'événements JSONL doit-il viser une convergence avec le vocabulaire d'un concurrent, ou assumer son propre vocabulaire documenté ? La recherche indique qu'aucun standard n'existe. À trancher avant US-017, car un changement ultérieur casserait les consommateurs.
[/PRD]
