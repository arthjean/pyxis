...
**Impact.** `AgentEvent` est deja `Serialize` (event.rs:11) et concu comme « LE contrat coeur → clients » : le mode `-p` jette cette valeur. Aucune orchestration externe (script, CI) ne peut suivre les appels d'outils, les permissions ou les compactions d'un run headless.
...
**Impact.** Tout second consommateur du coeur (futur mode serveur, tests d'integration) doit dupliquer la logique de correlation et de commandes. Une reponse tardive d'un tour annule ne peut etre rejetee que par une convention exterieure au contrat.
...
**Statut documentaire.** Assume et documente: ADR-7 R3 (docs/DECISIONS.md:201) et docs/CURRENT_STATUS.md:27 ('not a kernel-level network sandbox and does not block raw sockets by itself'). L ecart est connu, pas ignore, mais il reste un ecart de modele de securite.
...
**Statut documentaire.** ADR-7 R3 (docs/DECISIONS.md:201) assume Landlock FS process-wide; la consequence sur l escalade n est pas discutee.
...
**Statut documentaire.** Consequence directe d ADR-7 R3 (docs/DECISIONS.md:201) et du choix Linux-first; le compromis est coherent avec le budget de complexite du MVP.
...
**Statut documentaire.** docs/ROADMAP.md:117 place la memoire vectorielle sqlite-vec en Phase 2; docs/CURRENT_STATUS.md:21 la confirme comme differee
...
**Impact.** Aucune voie pour qu'un composant (hook, MCP, sous-systeme de statut) ajoute du contexte au modele en cours de session, et aucune economie de tokens: les blocs sont renvoyes integralement a chaque tour meme inchanges. Acceptable au perimetre actuel, bloquant des que les hooks arrivent.
...
**Statut documentaire.** docs/ROADMAP.md:114 differe les hooks utilisateur en Phase 2
...
**Statut documentaire.** docs/ARCHITECTURE.md:319 pose l'uniformité DynTool comme contrat cible mais ne traite pas le nommage ni les collisions.
...
**Statut documentaire.** docs/ARCHITECTURE.md:423 promet `returns_untrusted() == true` pour tous les outils MCP et le taint §4.6 (DECISIONS.md:203, risque R5), mais rien n'est implémenté: le taint est une défense post-exécution, pas une politique d'approbation pré-exécution.
...
**Statut documentaire.** docs/ARCHITECTURE.md:403-423 ne mentionne que les tools MCP; les resources ne sont ni prévues ni rejetées.
...
**Impact.** Impossible d'orchestrer Pyxis depuis un autre agent ou un IDE via MCP. À nuancer fortement: Pyxis n'a pas de surface app-server par choix (ADR-3), donc l'orchestration passe par d'autres voies.
...
**Statut documentaire.** Aucun ADR ne traite le mode serveur; docs/ARCHITECTURE.md:403-423 ne décrit MCP que comme consommation cliente (« Pyxis consomme MCP via rmcp »), ce qui suggère un choix implicite plutôt qu'un oubli.
...
**Impact.** Avec plusieurs sessions Pyxis ouvertes (usage multi-panes courant), rien ne distingue les onglets : ni le workspace, ni le statut du tour.
...
**Impact.** Consequence directe et acceptee du positionnement mono-consommateur. Le cout reel est qu'aucun ecosysteme tiers ne peut se construire sur Pyxis, et que meme un script Python de dogfooding devrait passer par `-p` en aveugle. A revoir seulement si Pyxis vise des integrateurs externes.
...
- **Aucun protocole de controle hors process pour un integrateur tiers** (mineur) : Pour un consommateur Rust in-process, l'API `Stream<AgentEvent>` est strictement superieure a du JSON-RPC : zero serialisation, types partages, annulation par drop. Le gap reel n'est donc pas le p
...
**Statut documentaire.** docs/ROADMAP.md:130-131 place la CI VCR en Phase 3 mais ne mentionne aucune CI de base pour les tests existants
...
**Statut documentaire.** docs/DECISIONS.md:138 et :203 designent les tests VCR comme la mitigation obligatoire de R4 ; docs/ROADMAP.md:131 les qualifie de 'non negociable, bloquant en CI' mais en Phase 3 ; docs/CURRENT_STATUS.md:24 les liste comme non livres
...
**Statut documentaire.** docs/ROADMAP.md annonce la review par hunk (TUI riche, Phase 2), qui presuppose ce tracker
...
##### Persistance des preferences non atomique et sans verrou, alors que N instances peuvent tourner en parallele

`moyen` · `divergent` · effort `S`

**Impact.** Deux panes Pyxis qui changent de modele ou de mode de permission au meme moment se perdent mutuellement leurs ecritures, et une interruption en plein `write` laisse un settings.toml tronque. Le scenario est structurel pour un agent concu pour tourner en plusieurs instances paralleles.
